                        ...
                        <div class="row ">
                            ...
                            <div class="col-md-4">
                                <fieldset class="form-group input-group-md">
                                    ...
                                    <input type="text" class="form-control" id="catalogueTitle" />
                                </fieldset>
                            </div>
                            <div class="col-md-4">
                                <fieldset class="form-group input-group-md">
                                    <label class="form-lbl"> File</label>
                                    <input type="file" class="form-control" id="attachment" accept="image/png, image/gif, image/jpeg"/>
                                </fieldset>
                            </div>
                    ...
                    <div class="card-footer">
                        <div class="row">
                            <div style="margin-bottom: 5px;" class="form-group col-md-12 text-right">
                                <button class="btn btn-danger btn-md" onclick="addUpdateCatalogue()"> Save </button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="row ">
                            ...
                            <div class="col-md-12">
                                <fieldset class="form-group input-group-md">
                                    <label class="form-lbl"> Catalogue List </label>
...
<script>
    ...
    function getUserMasterDetails() {
        ...
        let sendApiDataObj = {
            ...
            '<?php echo systemModuleFunction; ?>': 'getCatalogueDetails',
        ...
        APICallAjax(sendApiDataObj, function(response) {
            if (response.responseCode == resultOk) {
                let html = '';
                let count = 1;
                for (let i = 0; i < response.result.catalogue.length; i++) {
                    let catalogue = response.result.catalogue[i];

                    html += '<tr>';
                    html += '<td>'+count+'</td>';
                    html += '<td>'+catalogue.title+'</td>';
                    html += '<td><a target="_blank" href="'+catalogue.image+'">"'+catalogue.title+'"</a></td>';
                    html += '<td><button class="btn btn-danger btn-md" onclick="deletePortFolio("'+catalogue.image+'")"> Delete </button></td>';
                    html += '</tr>';
                    count++;
                }
    ...
    function addUpdateCatalogue() {
        let resultOk = '<?php echo resultOk; ?>';
        let file_data = $('#attachment')[0].files;
        let sendApiDataObj = {
            '<?php echo systemProject; ?>': 'Masters',
            '<?php echo systemModuleFunction; ?>': 'addUpdateCatalogue',
