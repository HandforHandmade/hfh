...
<script type="application/javascript">
    ...
    async function APICallAjax(SendDataObject, callback) {
        ...
        if (SendDataObject['ERPFileElement'] && SendDataObject.ERPFileElement.length > 0) {
            let data = new FormData();
            $.each(SendDataObject, function (key, input) {
                if(key != 'ERPFileElement') {
                    data.append(key, input);
                }
            });

            for (var i = 0; i < SendDataObject.ERPFileElement.length; i++) {
                data.append("ERPFileElement[]", SendDataObject.ERPFileElement[i]);
            }
            ...
            delete SendDataObject["ERPFileElement"];
            $.ajax({
                type: "POST",
                url: "<?php echo API_BASE_URL; ?>",
                headers: {
